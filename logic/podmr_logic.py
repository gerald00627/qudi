...
class ODMRLogic(GenericLogic):
    ...
    def on_activate(self):
        ...
        self.record_length_s = 3e-6 #def = 3e-6
    ...
    def start_odmr_scan(self):
        ...
        self.set_trigger(self.mw_trigger_pol, 5e-6)
    ...
    def _scan_odmr_line(self):
        ...
        for i in range(len(self.final_freq_list)):
            ...
            self._odmr_counter._pulser.pulser_on(n=self.lines_to_average, final=self._odmr_counter._pulser._laser_on_state)
            # self._odmr_counter._pulser.pulser_on(n=int(1), final=self._odmr_counter._pulser._laser_on_state)
            d =time.time()
            while True:    
        ...
        self.laser_data = self._odmr_counter._sc_device.get_measurements() #get_measurements('counts') shows keyerror
    ...
    def set_up_odmr(self, pi_pulse=100e-9):
        ...
        d_ch = clear(d_ch)
        d_ch[self._odmr_counter._pulser._mw_trig] = True
        block_1.append(init_length = 1e-3, channels = d_ch, repetition = 1)

        d_ch = clear(d_ch)
        d_ch[self._odmr_counter._pulser._mw1_switch] = True
        block_1.append(init_length = pi_pulse, channels = d_ch, repetition = 1)

        d_ch = clear(d_ch)
        d_ch[self._odmr_counter._pulser._laser_channel] = True
        d_ch[self._odmr_counter._pulser._pixel_start] = True
    ...
    def set_up_next_trigger(self):
        ...
        d_ch = clear(d_ch)
        # d_ch[self._odmr_counter._pulser._pixel_stop] = True
        d_ch[self._odmr_counter._pulser._mw_trig] = True
        block_1.append(init_length = 1e-3, channels = d_ch, repetition = 1)
    ...
    def analyse_pulsed_meas(self, analysis_settings, pulsed_meas):
        ...
        self.odmr_raw_data[0] = np.transpose(data)
